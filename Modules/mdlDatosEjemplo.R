...
datosEjemploServer <- function(input, output, session) {
  ...
  A4 <- rnorm(7, 96, 7)
  A5 <- rnorm(6, 90, 5)
  output$mediasMulTab1 <- renderTable(data.frame('Serie_1' = A1), digits = 2)
  output$mediasMulTab2 <- renderTable(data.frame('Serie_2' = A2), digits = 2)
  output$mediasMulTab3 <- renderTable(data.frame('Serie_3' = A3), digits = 2)
